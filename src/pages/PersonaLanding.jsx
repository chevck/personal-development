...
const featureCards = [
  {
    ...
    description: 'Pick what you want to improve—speech training is live; more programmes are on the way.',
    icon: (
  ...
  },
  {
    ...
    description: 'One focused exercise per calendar day. Set 7–60 days and build a rhythm that lasts.',
    icon: (
      <svg viewBox="0 0 24 24" className="h-7 w-7" fill="none" stroke="currentColor" strokeWidth="2">
  ...
  },
  {
    ...
    description: 'See your active week, completed days, and what to work on next—across every programme.',
    icon: (
      <svg viewBox="0 0 24 24" className="h-7 w-7" fill="none" stroke="currentColor" strokeWidth="2">
...
function PersonaMark({ className = '' }) {
  return (
    <span className={`relative inline-flex ${className}`}>
      <svg viewBox="0 0 40 40" className="h-full w-full" aria-hidden>
        <defs>
          <linearGradient id="persona-mark-grad" x1="0" y1="0" x2="1" y2="1">
            <stop offset="0%" stopColor="#7A57D1" />
            <stop offset="100%" stopColor="#3D2578" />
          </linearGradient>
        </defs>
        <circle cx="20" cy="20" r="17.5" fill="none" stroke="url(#persona-mark-grad)" strokeWidth="2.5" />
        <path d="M20 2.5 a17.5 17.5 0 0 0 0 35 z" fill="url(#persona-mark-grad)" />
      </svg>
      <span className="absolute -right-0.5 -top-0.5 h-2.5 w-2.5 rounded-full bg-persona-yellow ring-2 ring-white" />
    </span>
  );
}
...
function PersonaBrand({ size = 'md' }) {
  ...
  return (
    <Link to="/" className="group inline-flex items-center gap-2.5 no-underline" aria-label="Persona — home">
      <PersonaMark className={`${markSize} transition-transform duration-300 group-hover:rotate-[-8deg]`} />
      <span className={`${textSize} font-extrabold tracking-tight text-persona-ink`}>
        Persona
      </span>
    </Link>
  );
...
function SquiggleUnderline() {
  return (
    <svg
      className="mx-auto mt-1 h-3 w-40 text-persona-purple md:w-52"
      viewBox="0 0 200 12"
      fill="none"
      aria-hidden
    >
...
function StarburstIcon({ children, light = false }) {
  return (
    ...
    >
      <svg
        className="star-twinkle absolute inset-0 h-full w-full"
        viewBox="0 0 64 64"
        aria-hidden
      >
        <path
...
function CardPattern({ type, light = false }) {
  ...
  if (type === 'dots') {
    return (
      <svg
        className="card-pattern pointer-events-none absolute -right-2 -top-2 h-24 w-24 opacity-80"
        viewBox="0 0 80 80"
        aria-hidden
      >
        {[0, 1, 2].map((row) =>
  ...
  if (type === 'rings') {
    return (
      <svg
        className="card-pattern pointer-events-none absolute -right-4 bottom-0 h-28 w-28"
        viewBox="0 0 100 100"
        aria-hidden
      >
        <circle cx="50" cy="50" r="40" stroke={stroke} strokeWidth="2" fill="none" />
        <circle cx="50" cy="50" r="28" stroke={stroke} strokeWidth="2" fill="none" />
        <circle cx="50" cy="50" r="16" stroke={stroke} strokeWidth="2" fill="none" />
      </svg>
    );
  }
  return (
    <svg
      className="card-pattern pointer-events-none absolute -left-2 bottom-0 h-20 w-32 opacity-70"
      viewBox="0 0 120 40"
      aria-hidden
    >
      <path
...
function ProgressRing({ value = 62, size = 52, stroke = 6 }) {
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} className="-rotate-90" aria-hidden>
      <circle cx={size / 2} cy={size / 2} r={radius} fill="none" stroke="rgba(94,58,173,0.15)" strokeWidth={stroke} />
      <circle
        cx={size / 2}
        cy={size / 2}
        r={radius}
...
function HeroShowcase() {
  return (
    <div className="hero-in hero-in-delay-3 relative mx-auto mt-14 w-full max-w-sm lg:mt-0 lg:max-w-md">
      <div
        className="hero-blob pointer-events-none absolute -left-10 top-2 -z-10 h-48 w-48 rounded-full bg-persona-lavender-deep/60 blur-3xl"
        aria-hidden
      />
      <div
        className="hero-blob-alt pointer-events-none absolute -right-8 bottom-2 -z-10 h-44 w-44 rounded-full bg-persona-yellow/40 blur-3xl"
        aria-hidden
      />
      ...
      <div
        ...
        aria-hidden
      >
        <img src={HERO_PHOTOS.practise} alt="" className="h-full w-full object-cover" />
      </div>
      <div
        ...
        aria-hidden
      >
        <img src={HERO_PHOTOS.learn} alt="" className="h-full w-full object-cover" />
      </div>
      ...
      <div className="card-float relative z-10 rounded-[1.75rem] border border-persona-lavender-deep/60 bg-white/95 p-6 shadow-[0_28px_70px_rgba(94,58,173,0.25)] backdrop-blur">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-persona-lavender">
              <AppLogo projectId={SPEECH_TRAINING_PROJECT_ID} variant="icon" size="sm" className="h-6 w-6" />
            </span>
            <div>
              <p className="text-sm font-extrabold text-persona-ink">Speakly</p>
              <p className="text-xs font-semibold text-persona-muted">Day 3 of 21</p>
            </div>
          </div>
          <span className="rounded-full bg-persona-yellow/40 px-3 py-1 text-xs font-bold text-persona-purple-dark">
            Today
          </span>
        </div>
        ...
        <div className="mt-5 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs font-semibold text-persona-muted">Programme</p>
      ...
      <div className="chip-float absolute -left-5 top-6 z-20 rounded-2xl border border-persona-lavender-deep/60 bg-white px-3.5 py-2.5 shadow-[0_12px_30px_rgba(94,58,173,0.2)]">
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="text-[11px] font-semibold leading-tight text-persona-muted">Streak</p>
            <p className="text-sm font-extrabold leading-tight text-persona-ink">5 days</p>
          </div>
        </div>
      </div>
      ...
      <div className="chip-float-delayed absolute -bottom-4 right-2 z-20 rounded-2xl border border-persona-lavender-deep/60 bg-white px-3.5 py-2.5 shadow-[0_12px_30px_rgba(94,58,173,0.2)]">
        <div className="flex items-center gap-2.5">
          <span className="flex h-8 w-8 items-center justify-center rounded-full bg-green-100 text-green-600" aria-hidden>
            <svg viewBox="0 0 20 20" className="h-4 w-4" fill="none" stroke="currentColor" strokeWidth="2.5">
              <path d="M4 10l4 4 8-9" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
          </span>
          <div>
            <p className="text-[11px] font-semibold leading-tight text-persona-muted">Assessor feedback</p>
            <p className="text-sm font-extrabold leading-tight text-persona-ink">Scored 8/10</p>
          </div>
        </div>
      </div>
    </div>
...
function FeaturedSkillCard({ skill }) {
  ...
  return (
    <li ref={ref} className={`reveal md:col-span-3 ${visible ? 'is-visible' : ''}`}>
      <Link
      ...
      >
        ...
        <div className="flex items-start justify-between gap-3">
          <StarburstIcon light>
            <AppLogo projectId={skill.id} variant="icon" size="sm" className="h-7 w-7" />
          </StarburstIcon>
          <span className="shrink-0 rounded-full bg-persona-yellow px-3 py-1 text-xs font-bold uppercase tracking-wide text-persona-ink">
            {skill.tag}
          </span>
        </div>
        ...
        <div className="mt-auto flex items-center justify-between gap-4 pt-8">
          <span className="text-sm font-semibold text-persona-yellow">{skill.durationLabel}</span>
          <span className="inline-flex items-center gap-2 text-sm font-bold text-white">
            Explore {skill.name}
            <span
              className="flex h-9 w-9 items-center justify-center rounded-full bg-white/20 transition group-hover:translate-x-1 group-hover:bg-white/30"
              aria-hidden
            >
              <svg viewBox="0 0 20 20" className="h-4 w-4" fill="none" stroke="currentColor" strokeWidth="2.5">
...
function ComingSoonSkillCard({ skill }) {
  ...
  return (
    <li ref={ref} className={`reveal reveal-stagger-1 md:col-span-2 ${visible ? 'is-visible' : ''}`}>
      <div
      ...
      >
        ...
        <div className="flex items-start justify-between gap-3">
          <span className="flex h-12 w-12 items-center justify-center rounded-2xl bg-persona-lavender text-xl">
            {skill.icon}
          </span>
          <span className="shrink-0 rounded-full bg-white px-3 py-1 text-xs font-bold uppercase tracking-wide text-persona-muted ring-1 ring-persona-lavender-deep">
            {skill.tag}
          </span>
        </div>
        ...
        <p className="mt-2 text-sm leading-relaxed text-persona-muted">{skill.description}</p>
        <div className="mt-auto flex items-center justify-between gap-3 pt-6">
          <span className="text-xs font-semibold text-persona-muted">{skill.durationLabel}</span>
...
export default function PersonaLanding() {
  ...
  return (
    <div className="persona-landing relative min-h-screen overflow-x-hidden bg-persona-cream font-sans text-persona-ink">
      <div
      ...
      <header className="sticky top-0 z-50 border-b border-persona-lavender-deep/60 bg-white/85 backdrop-blur-lg">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-6 px-6 py-4">
          ...
          <a href="#skills" className="group btn-cta text-sm">
        ...
        <div className="mx-auto grid max-w-6xl items-center gap-12 lg:grid-cols-[1.05fr_0.95fr] lg:gap-8">
          <div className="text-center lg:text-left">
            <span className="hero-in hero-in-delay-1 inline-flex items-center gap-2 rounded-full bg-persona-lavender px-4 py-1.5 text-sm font-bold text-persona-purple">
              <span className="h-2 w-2 animate-pulse rounded-full bg-persona-yellow" />
            ...
            <p className="hero-in hero-in-delay-3 mx-auto mt-6 max-w-xl text-lg leading-relaxed text-persona-muted md:text-xl lg:mx-0">
              Persona is your home for focused growth—choose a skill, set 7–60 days, and practise
              one exercise per day. Speech training is live; more skills are coming soon.
            </p>

            <div className="hero-in hero-in-delay-4 mt-9 flex flex-wrap items-center justify-center gap-4 lg:justify-start">
              <a href="#skills" className="group btn-cta text-base">
              ...
              <Link to="/speakly/welcome" className="btn-secondary px-8 py-3.5 text-base">
                Meet Speakly
              </Link>
            </div>

            <ul className="mt-10 flex items-center justify-center gap-5 sm:gap-7 lg:justify-start">
              {HERO_STATS.map((stat, i) => (
                <li
                  key={stat.label}
                  className="hero-in flex items-center gap-5 sm:gap-7"
                  style={{ animationDelay: `${0.55 + i * 0.08}s`, opacity: 0 }}
                >
                  {i > 0 && <span className="h-9 w-px bg-persona-lavender-deep" aria-hidden />}
                  <div className="text-center lg:text-left">
                    <p className="font-display text-2xl font-medium text-persona-purple sm:text-3xl">
                      {stat.value}
                    </p>
      ...
      <section className="relative px-6 py-12 md:py-20">
        <RevealSection className="mx-auto max-w-6xl text-center">
          <span className="inline-block rounded-full bg-persona-yellow/40 px-4 py-1 text-xs font-bold uppercase tracking-wider text-persona-purple-dark">
            How Persona helps
          </span>
          <h2 className="mt-4 text-3xl font-extrabold tracking-tight text-persona-ink md:text-4xl">
            Learn,{' '}
            <span className="font-accent text-4xl text-persona-purple md:text-5xl">practise</span>
            ,{' '}
            <span className="font-accent text-4xl text-persona-yellow md:text-5xl">progress</span>
          </h2>
        </RevealSection>

        <ul
          ref={featuresRef}
          className="mx-auto mt-12 grid max-w-6xl gap-6 md:grid-cols-3 md:gap-8"
        >
          {featureCards.map((card, i) => (
            ...
            >
              ...
              <h3 className="mt-6 text-2xl font-extrabold leading-tight">
                {card.title}{' '}
                <span className={`font-accent text-3xl font-bold ${card.accentColor}`}>
                  {card.accent}
                </span>
              </h3>
              <p
      ...
      <section id="skills" className="relative px-6 py-16 md:py-24">
        <RevealSection className="mx-auto max-w-6xl text-center">
          ...
          <h2 className="mt-4 text-3xl font-extrabold tracking-tight text-persona-ink md:text-4xl">
            Choose what to{' '}
            <span className="font-accent text-4xl text-persona-purple md:text-5xl">work on</span>
          </h2>
          <p className="mx-auto mt-4 max-w-2xl text-base text-persona-muted">
            Each skill is a guided programme with daily exercises. Speech training is live;
            content creation is on the way.
          </p>
        </RevealSection>

        <ul className="mx-auto mt-14 grid max-w-5xl items-stretch gap-6 md:grid-cols-5">
      ...
      <section id="how-it-works" className="relative overflow-hidden px-6 py-20">
        ...
        <RevealSection className="relative mx-auto max-w-3xl text-center">
          <h2 className="text-3xl font-extrabold text-persona-ink md:text-4xl">
            How Persona{' '}
            <span className="font-accent text-4xl text-persona-purple md:text-5xl">works</span>
          </h2>
        </RevealSection>
        <ol className="relative mx-auto mt-12 max-w-3xl space-y-5">
          {steps.map((item, i) => (
            ...
            >
              ...
              <div>
                <h3 className="text-lg font-bold text-persona-ink">{item.title}</h3>
      ...
      <section id="why" className="px-6 py-20 md:py-28">
        <RevealSection className="mx-auto max-w-6xl text-center">
          <h2 className="text-3xl font-extrabold text-persona-ink md:text-4xl">
            Built for{' '}
            <span className="font-accent text-4xl text-persona-yellow md:text-5xl">
              lasting growth
            </span>
          </h2>
        </RevealSection>
        <ul className="mx-auto mt-14 grid max-w-6xl gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {[
            ...
            >
              ...
              <h3 className="mt-4 text-lg font-bold text-persona-ink">{feature.title}</h3>
      ...
      <section className="px-6 py-20" aria-labelledby="cta-speech-training">
        <div className="relative mx-auto max-w-4xl overflow-hidden rounded-[2.5rem] px-8 py-14 text-center md:px-16">
          ...
          />
          <div
            className="card-pattern pointer-events-none absolute -right-8 -top-8 h-40 w-40 rounded-full bg-persona-yellow/20 blur-2xl"
            aria-hidden
          />
          <div className="relative z-10">
            <h2 id="cta-speech-training" className="text-3xl font-extrabold text-white md:text-4xl">
              Start with{' '}
              <span className="font-accent text-4xl text-persona-yellow md:text-5xl">speech training</span>
            </h2>
            <p className="mx-auto mt-4 max-w-lg text-base text-white/90">
              Speakly is our first Persona skill—pace, clarity, and delivery with assessor feedback.
              Create an account and choose how many days you want to train.
            </p>
            <div className="mt-8 flex flex-wrap items-center justify-center gap-4">
              <Link to="/speakly/register" className="group btn-cta-light text-base no-underline">
              ...
              <Link to="/speakly/welcome" className="btn-cta-outline no-underline">
                Learn about Speakly
              </Link>
            </div>
          </div>
      ...
      <footer className="border-t border-persona-lavender-deep bg-white/80 px-6 py-10 text-center text-sm text-persona-muted backdrop-blur-sm">
        <div className="mb-3 flex justify-center">
          <PersonaBrand size="sm" />
        </div>
        <p>© {new Date().getFullYear()} Persona · Self-development, one skill at a time</p>
      </footer>
    </div>
  );
}
